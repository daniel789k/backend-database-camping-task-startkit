...
SELECT COUNT(*) as 員工總數 FROM "USER";
...
(
    ...
    (select credit_amount from "CREDIT_PACKAGE" where credit_amount = '14'),
    (select price from "CREDIT_PACKAGE" where credit_amount = '14')
),
(
    (select id from "USER" where email = 'wXlTq@hexschooltest.io'),
    (select id from "CREDIT_PACKAGE" where credit_amount = '21'),
    (select credit_amount from "CREDIT_PACKAGE" where credit_amount = '21'),
...
SELECT
  ...
  sum("CREDIT_PURCHASE".purchased_credits) - count(distinct "COURSE_BOOKING".join_at) as remaining_credit
FROM "COURSE_BOOKING"
...
ORDER BY "COACH".experience_years ASC;
...
FROM "CREDIT_PURCHASE"
where purchase_at >= '2024-12-01 00:00:00'
